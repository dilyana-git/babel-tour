...
// would number its rooms, not read a progress meter.
const ROMAN = ['I', 'II', 'III', 'IV', 'V', 'VI', 'VII', 'VIII'];
const MARKS = NODES.map((node, i) => {
  const garden = i > LIBRARY_MAX;
  const within = garden ? i - LIBRARY_MAX : i + 1;
  return `${node.title.toUpperCase()} — ${garden ? 'PATH' : 'GALLERY'} ${ROMAN[within - 1]}`;
});

// The quill cadence (design: Quote Treatments, 1b) — an unseen hand writes
// each folio line onto the artwork letter by letter. Strokes land 58–104ms
// apart, words breathe a beat longer, and the pen rests after punctuation.
// The jitter is a tiny seeded PRNG so every visit is written in the same hand.
const mulberry = (seed) => () => {
  seed |= 0;
...
// No quote may take longer than this to write itself. At the natural stroke
// rate the 130-character Web of Time runs ~14s — far longer than anyone stands
// still — so long lines are written in a brisker hand rather than a slower
// reader. The short quotes finish well inside the budget and keep their pace.
const QUILL_BUDGET_MS = 6500;

const QUILL = NODES.map((node) => {
  const rand = mulberry(7331);
  let t = 0;
  let words = node.folio.line.split(' ').filter(Boolean).map((word) => {
    ...
    t += 120 + rand() * 90;
    if (/[,;.!?—"”]$/.test(word)) t += 380; // the pen rests
    return letters;
  });
  if (t > QUILL_BUDGET_MS) {
    const hurry = QUILL_BUDGET_MS / t;
    words = words.map((ls) => ls.map((l) => ({ ...l, d: l.d * hurry })));
    t = QUILL_BUDGET_MS;
  }
  words = words.map((ls) => ls.map((l) => ({ ...l, d: Math.round(l.d) })));
  // The quotes run from 45 to 130 characters. One size can't hold that range
  // on the artwork: the short ones want to be grand, the long ones would
  // sprawl into the depth dots. Each line gets the largest hand it can wear
  // and still sit in two or three written lines.
  const n = node.folio.line.length;
  const size = n <= 55
    ? 'clamp(2.4rem, 4.1vw, 5rem)'
    : n <= 95
      ? 'clamp(2rem, 3.3vw, 3.9rem)'
      : 'clamp(1.65rem, 2.6vw, 3.1rem)';
  // The gold rule draws itself under the finished line; the attribution
  // surfaces last.
  return { words, size, ruleD: Math.round(t) + 300, attrD: Math.round(t) + 1100 };
});
...
export default function Tour() {
  ...
  useEffect(() => {
    ...
    const tick = (now) => {
      ...
      if (settled && !diving) {
        ...
        // units/s over the 15-unit APPROACH) so a step-in moves at constant
        // walking tempo; near arrival the exponential takes over for the stop.
        const IMMERSION_WALK = 0.15; // immersion units per second
  ...
  // The threshold whisper: shown while the reader stands at the open door. It
  // takes the nav hint's place on screen, so the hint stands down while it's up.
    ...
    >
      ...
      <div className="folio">
        <FadeSwap
          id={chapter}
          render={(i) => {
            const { words, size, ruleD, attrD } = QUILL[i];
            return (
              <>
                <div className="folio-scrim" aria-hidden="true" />
                <div className="folio-eyebrow">{NODES[i].folio.eyebrow}</div>
                <div className="folio-line" style={{ fontSize: size }}>
                  <span className="sr-only">{NODES[i].folio.line}</span>
                  <span aria-hidden="true">
                    {words.map((letters, w) => (
                      <span key={`${i}-${w}`} className="folio-word">
                        {letters.map(({ ch, d }, l) => (
                          <span
                            key={l}
                            className="folio-ink"
                ...
                <div
                  className="folio-rule"
                  aria-hidden="true"
                  style={{ animationDelay: `${ruleD}ms` }}
                />
                <div className="folio-attr" style={{ animationDelay: `${attrD}ms` }}>
                  {NODES[i].folio.attr}
                </div>
              </>
            );
      ...
      {/* The controls, handwritten in the right margin (design: 3a). Each is a
          phrase rather than a labelled pill, and a manicule ☞ marks whatever is
          currently on — the same mark a reader would pencil beside a line. */}
      <nav className="margin-nav" aria-label="Move through the corridor">
        <button
          type="button"
          className="margin-act"
          aria-label="Ascend one gallery"
          title="Ascend one gallery (↑)"
          onClick={(e) => { e.stopPropagation(); go(-1); }}
          disabled={chapter === 0}
        >
          ascend <span className="margin-key" aria-hidden="true">↑</span>
        </button>
        <button
          type="button"
          className={`margin-act${autoplay ? ' is-on' : ''}`}
          aria-label={autoplay ? 'Pause the drift' : 'Drift downward on its own'}
          aria-pressed={autoplay}
          onClick={(e) => { e.stopPropagation(); setAutoplay((v) => !v); }}
        >
          {autoplay && <span className="margin-hand" aria-hidden="true">☞</span>}
        ...
        >
          descend <span className="margin-key" aria-hidden="true">↓</span>
        </button>
        <button
          type="button"
          className={`margin-act${muted ? ' is-on' : ''}`}
          aria-label={muted ? 'Unmute the ambience' : 'Mute the ambience'}
          aria-pressed={muted}
          onClick={(e) => { e.stopPropagation(); toggleMute(); }}
        >
          {muted && <span className="margin-hand" aria-hidden="true">☞</span>}
          {muted ? 'wake the choir' : 'hush the choir'}
        </button>
        <button
          type="button"
          className={`margin-act${showHelp ? ' is-on' : ''}`}
        ...
        >
          {showHelp && <span className="margin-hand" aria-hidden="true">☞</span>}
          lost? <span className="margin-key" aria-hidden="true">H</span>
        </button>
      </nav>
      ...
      {/* No button at the threshold — the walk itself is the trigger (any
          ...
          whisper only names the change, standing in for the nav hint in the
          same spot and voice (the hint yields below while it is up). */}
      {whisperShown && (
